...
public class Methods {
	...
	public static String substituteAll(String source, char target, String replacement) {
		String result = "";
		char [] newMatrix = source.toCharArray();
		for (int i = 0; i< source.length(); i++) {
			if (newMatrix[i] == target) {
				result += replacement;
			}
			else {
				result += newMatrix[i];
	 ...
	 * 
	 * @param values of integers
	 * @return the mean of the elements in array
	 */
	public static double arrayMean (int[] values) {
		double x = (double)arraySum (values) / (double)(values.length);
		return x;
	}
